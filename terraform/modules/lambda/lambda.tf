resource "aws_lambda_function" "queued-transactions-lambda" {
  ...
  handler       = "apps/queued-transactions/main.handler"
  role          = aws_iam_role.transactions-processing-policy.arn
  runtime       = "nodejs20.x"
  timeout       = "30"

  // source_code_hash = data.archive_file.transactions-processing-api.output_base64sha256
  s3_bucket = "hot-reload"
  s3_key    = abspath("${path.root}/../dist")
...
/* resource "aws_lambda_function" "receive-transactions-lambda" {
  ...
  handler       = "apps/${var.receive-transactions}/main.handler"
  role          = aws_iam_role.transactions-processing-policy.arn
  runtime       = "nodejs20.x"
  timeout       = "10"
